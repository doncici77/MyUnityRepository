...
public class AudioBoardVisualizer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // GetComponentsInChildren<T>는 오브젝트가 연결된 자식 컴포넌트들을 가져오는 코드
        // 현제 코드 기준으로는 Board의 배열
    ...
    void Update()
    {
        ...
        // samples = FFT(신호에 대한 주파수 영역)을 저장할 배열, 이 배열 값은 2의 제곱 수로 표현함.
        // 채널은 최대 8개의 채널을 지원해주고 있음. 일반적으로는 0을 사용합니다.
        // FFTWindow는 샘플링 진행할 때 쓰는 값

        // 보드의 각각의 개수만큼 작업을 진행합니다.
        for (int i = 0; i < boards.Length; i++)
        {
            ...
            // 여기서 3.0f는 막대에 대한 수치 보정값

            boards[i].GetComponent<RectTransform>().sizeDelta = size;
            // sizeDelta는 부모를 기준으로 크기가 얼마나 큰지 작은지를 나타낸 수치를 의미함.
        }
    }
}
