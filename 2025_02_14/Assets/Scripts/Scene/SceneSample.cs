...
public class SceneSample : MonoBehaviour
{
    ...
    // 유니티에서는 시작 단계부터 종료 단계까지를 함수로 제공함.
    // ex) Awake(시작 전), Start(시작), Update(진행중) ...

    // 오브젝트(컴포넌트)가 활성화 되었을 경우
    ...
    {
        Debug.Log($"현제 로드된 씬의 이름은 {scene.name}입니다.");
    }
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.I))
        {
            ...
            // LoadSceneMode가 Additive일 경우는 기존 씬 위에 새로운 씬을 중복해서 로드하느 설정
