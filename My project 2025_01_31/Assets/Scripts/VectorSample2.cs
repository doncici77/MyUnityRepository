...
{
    // VectorSample(스크립트파일)과 같이 공부하면 좋음.
    ...
    void Start()
    {
        ...
        // 일반적인 단일 방향 보다 이동속도가 더 빠른 상황이 발생할 수 있음.
        ...
        // >> 끝 점의 값이 제공되었을 때 그 사이에 이치한 값을 추정하기 위해 직선거리에 따라 선형적으로 계산하는 방식
        Vector3 Result = Vector3.Lerp(positionA, positionB, 0.5f);

        // ex) A지점이 (2,1)이고 B지점이 (6,4)일 때 그 사이의 지점 C의 X좌표가 주어진다면, Y좌표까지 얻어낼 수 있습니다.
    }
