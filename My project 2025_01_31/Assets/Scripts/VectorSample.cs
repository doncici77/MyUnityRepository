...
public class VectorSample : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 순서가 변경되어도 결과는 같습니다.
        // 특정 포지션에서 점프한 느낌의 계산 처리 // 포탈 혹은 위치이동
        Vector3 c = a + b;
        var trap_air = some + new Vector3(0, point);
        // var는 C#에서 값을 기준으로 데이터를 자동으로 설정하는 키워드
        // 현제 코드 기준으로는  뒤의 값이 Vector3이기 때문에 var는 Vector3
        ...
        // 이 거리를 측정 후 지정한 거리와 같거나 가깝다면 공격해라와 같은 코드를 짜기 좋음.

        // 곱셈
        // 벡터의 각 성분에 스칼라 값을 곱함.
        // 벡터 * 스칼라 -> 원본과 동일한 방향을 가리키는 벡터
        // 방향 자체에는 영향 안주고 벡터의 크기를 변경하는 경우에 사용함.
        Vector3 e = a * 2;
