...
// 유니티에서 특정 데이터 또는 기능을 구현하기 위해 적합한 자료형을 고르는 건 필수이다.
// 기본 자료형 이욍에 특정 기능, 작업을 진행할 수 있는 데이터 집합체를 자료구조라 부르겠다.(데이터 값의 모임)
...
// 5. 해시셋(HashSet): 데이터의 중복을 전혀 허락하지 않는 경우, 정령 순서가 필요 없는 경우

public class DataStructureSample : MonoBehaviour
{
    // 큐(Queue)
    // 제공해주는 기능: 삽입, 삭제, 첫번째 값 조회 기능
    // 단점: 중간에 있는 데이터를 접근하는 부분에선 매우 비효율적이다.
    ...
    {
        // 1) 큐에 데이터 추가
        stringQueue.Enqueue("나좀 도와주세요.ㅠㅠ");
        stringQueue.Enqueue("무슨 일이죠?");
        stringQueue.Enqueue("바나나 껍질 20개가 필요해요. :D");
        ...
        {
            Debug.Log(stringQueue.Peek()); // 큐에 저장된 가장 맨 앞의 값을 return한다.
        }
